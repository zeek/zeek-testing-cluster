...
if [[ $do_build = no ]]; then
    ...
    # zeektest/zeek progression. This technically takes the start time, but the
...
# (zeektest/build). The final step is a multi-stage build continuation that
# grabs the installed build into a slimmer final image (zeektest/zeek).
...
docker commit zeek-builder zeektest/build
docker rm -f zeek-builder 2>/dev/null
